...
#define kSpace      10
#define kHeight     16
#define kSWidth (kScreenWidth * 0.72)
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.outDateColor = UIColorFromRGB(0x565656);
        self.inDateColor = UIColorFromRGB(0x565656);
...
- (void)layoutSubviews {
    self.toolBarView.frame = CGRectMake(0, 0, kSWidth, kSpace + kHeight);
    
    self.weeklyView.frame = CGRectMake(0, CGRectGetMaxY(self.toolBarView.frame) + 5, kSWidth, kHeight + 10);
    self.weeklyView.layer.cornerRadius = self.weeklyView.hd_height / 2;
    self.weeklyView.layer.masksToBounds = YES;
    
    CGFloat width = (kSWidth - (kWeeklyDays + 1) * kSpace) / kWeeklyDays;
    int i = 0;
    for (UILabel *label in self.weeklyView.subviews) {
        label.frame = CGRectMake((kSpace + width) * i + kSpace, 5, width, kHeight);
        i++;
    }
    
    self.collectionView.frame = CGRectMake(0, CGRectGetMaxY(self.weeklyView.frame) + 5, kSWidth, self.height - CGRectGetMaxY(self.weeklyView.frame) - 10);
}
...
//            cell.dateLabel.layer.borderColor = kMainColor.CGColor;
//            cell.dateLabel.layer.borderWidth = 2;
        ...
        NSString * currentDateSgtr = [formatter stringFromDate:todayDate];
        
        BOOL isHave = NO;
        for (NSString * dateStr in self.studayDateArray) {
            if ([currentDateSgtr isEqualToString:dateStr]) {
                isHave = YES;
                break;
            }
        }
...
- (UIButton *)rightButton {
    if (!_rightButton) {
        UIButton *rightBtn = [[UIButton alloc] initWithFrame:CGRectMake(kSWidth - kSpace * 2 - kHeight, 5, kHeight, kHeight)];
...
- (UIButton *)dateButton {
    if (!_dateButton) {
        UIButton *dateButton = [[UIButton alloc] initWithFrame:CGRectMake((kSWidth - 8 * kSpace) / 2.0, 5, 8 * kSpace, kHeight)];
...
- (UICollectionView *)collectionView {
    if (!_collectionView) {
        UICollectionView *view = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, kSWidth, kScreenHeight * 0.556) collectionViewLayout:[[UICollectionViewLayout alloc] init]];
...
{
    self.studayDateArray = dataArray;
