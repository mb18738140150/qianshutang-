...
- (instancetype)initWithChatroom:(NIMChatroom *)room
{
    if (self = [super init]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(joinConferenceAction) name:kNTESJoinWhiteBoardNotification object:nil];
    }
    return self;
}
...
- (void)onOpenDocumentPressed:(id)sender
{
    ...
    docVc.delegate = self;
    [self presentViewController:docVc animated:NO completion:nil];
}
