...
#define kRightViewTag   10000
#define kLeftViewTag   20000
...
- (instancetype)initWithMeetingroom:(NIMNetCallMeeting *)meeting{
    ...
    if (self) {
        _meeting = meeting;
        _chatroom = [[NIMChatroom alloc]init];
        _chatroom.roomId = meeting.name;
        if ([[UserManager sharedManager] getUserType] == UserType_teacher) {
            _chatroom.creator = [NSString stringWithFormat:@"%d",[[UserManager sharedManager] getUserId]];
        }else
        {
            _chatroom.creator = @"5";
        }
...
- (NTESMeetingActorsView *)actorsView{
    ...
    if (!_actorsView) {
        _actorsView = [[NTESMeetingActorsView alloc] initWithFrame:CGRectMake(0, 0, self.view.width / 2, self.view.height)];
...
- (NTESMeetingActionView *)actionView
{
    ...
    if (!_actionView) {
        ...
        _actionView.frame = CGRectMake(kScreenWidth / 2, 0, kScreenWidth / 2, kScreenHeight);
...
- (void)chatroomBeKicked:(NIMChatroomBeKickedResult *)result
{
    if ([result.roomId isEqualToString:self.chatroom.roomId]) {
        ...
        DDLogInfo(@"chatroom be kicked, roomId:%@  rease:%zd",result.roomId,result.reason);
...
- (void)setupBarButtonItem
{
    ...
    UIView * oldLeftView = [self.view viewWithTag:kLeftViewTag];
    if ([self.view.subviews containsObject:oldLeftView]) {
...
-(void)refreshTecNavBar
{
    ...
    UIView * rightView = [[UIView alloc]initWithFrame:CGRectMake(kScreenWidth / 2 - 3*btnMargin-3*btnWidth, 0, 3*btnMargin+3*btnWidth, 30)];
    ...
    UIView * oldRightView = [self.view viewWithTag:kRightViewTag];
    if ([self.view.subviews containsObject:oldRightView]) {
...
-(void)refreshStdNavBar
{
    ...
    if (myRole.isActor&&!_isRemainStdNav) {  //有发言权限，变成3个按钮
        UIView * rightView = [[UIView alloc]initWithFrame:CGRectMake(kScreenWidth / 2 - 4*(btnWidth+btnMargin), 0,4*(btnWidth+btnMargin), btnHeight)];
        //视频按钮
        UIButton *videoButton = [UIButton buttonWithType:UIButtonTypeCustom];
        videoButton.frame = CGRectMake(2*btnMargin+btnWidth, 0, btnWidth, btnHeight);
        [videoButton setImage:[UIImage imageNamed:videoImage] forState:UIControlStateNormal];
    ...
    {
        UIView * rightView = [[UIView alloc]initWithFrame:CGRectMake(kScreenWidth / 2 - 2*(btnWidth+btnMargin), 0, 2*(btnWidth+btnMargin), btnHeight)];
        //互动按钮
...
- (void)requestCloseChatRoom
{
    ...
    //获取当前会议
    NIMNetCallMeeting *meeting = self.meeting;
