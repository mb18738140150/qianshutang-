...
- (void)initCellWithAssetString:(PHAsset *)asset  isCancleAction:(BOOL)cancleAction{
    ...
    self.timeLB.text = [self getTime:asset];
...
- (NSString *)getTime:(PHAsset *)asset
{
    
    int minit = (int)asset.duration / 60;
    int sec = (int)asset.duration % 60;
    if (sec >= 10) {
        return [NSString stringWithFormat:@"%d:%d", minit, sec];
    }else
    {
        return [NSString stringWithFormat:@"%d:0%d", minit, sec];
    }
}
