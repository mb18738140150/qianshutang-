...
-(void)initDataProgress:(BLSelectImageLoading)progressBlock finished:(BLSelectImageFinishedBlock)finishedBlock cancle:(BLCancle)cancleBlock{
    ...
    [BLImageHelper shareImageHelper].isAllVideo = NO;
...
-(void)initVideoDataProgress:(BLSelectImageLoading)progressBlock finished:(BLSelectImageFinishedBlock)finishedBlock cancle:(BLCancle)cancleBlock
{
    _progressBlock = progressBlock;
    _finishedBlock = finishedBlock;
    _cancleBlock = cancleBlock;
    [BLImageHelper shareImageHelper].isAllVideo = YES;
    [[BLImageHelper shareImageHelper].phassetChoosedArr removeAllObjects];
    [[BLImageHelper shareImageHelper] getAllVideoActionWithCollection:nil];
}
...
-(void)checkPhotoAbluPermissions{
    ...
    if (i==-1) {
        ...
        [[BLImageHelper shareImageHelper].phassetChoosedArr removeAllObjects];
        if ([BLImageHelper shareImageHelper].isAllVideo) {
            [[BLImageHelper shareImageHelper] getAllVideoActionWithCollection:nil];
        }else
        {
            [[BLImageHelper shareImageHelper] getAllImageActionWithCollection:nil];
    ...
    if (indexPath.item == 0 && self.showCamera && [BLImageHelper shareImageHelper].isAllphoto) {
        ...
        if (self.maxNum == 1) {
            ...
            if ([BLImageHelper shareImageHelper].isAllVideo) {
                ...
                [[BLImageHelper shareImageHelper].phassetChoosedArr addObject:[BLImageHelper shareImageHelper].phassetArr[indexPath.item]];
                [weakSelf sendData];
                
//                LibraryVideoPlayViewController * playVC = [[LibraryVideoPlayViewController alloc]init];
//                playVC.asset = [BLImageHelper shareImageHelper].phassetArr[indexPath.item];
//                playVC.SelectBlock = ^{
//                    [[BLImageHelper shareImageHelper].phassetChoosedArr addObject:[BLImageHelper shareImageHelper].phassetArr[indexPath.item]];
//                    [weakSelf sendData];
//                };
